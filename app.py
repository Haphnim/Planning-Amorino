...
def obtenir_mot_de_passe_responsable():
    """st.secrets lève StreamlitSecretNotFoundError (pas juste un dict vide)
    quand aucun fichier secrets.toml n'existe du tout sur la machine, ce qui
    fait planter l'appli au lieu de retomber sur le mot de passe par défaut.
    On protège donc l'accès."""
    try:
        return st.secrets.get("RESPONSABLE_PASSWORD", DEFAULT_RESPONSABLE_PASSWORD)
    except Exception:
        return DEFAULT_RESPONSABLE_PASSWORD
...
def md_html(s):
    """st.markdown(unsafe_allow_html=True) mais en retirant l'indentation de
    CHAQUE ligne individuellement (pas juste le préfixe commun façon dedent,
    qui échoue dès qu'une ligne interpolée comme LOGO_SVG a sa propre
    indentation à 0). Sinon Markdown interprète les lignes indentées de 4+
    espaces comme un bloc de code littéral et affiche le HTML brut au lieu de
    le rendre (c'est le bug qui affichait le <div>...</div> en clair à l'écran).
    Sûr ici car ce contenu est du HTML/CSS, jamais un <pre> où l'indentation
    compterait."""
    lignes = [ligne.strip() for ligne in s.strip("\n").split("\n")]
...
def page_connexion():
    ...
    c1, c2 = st.columns(2)
    with c1:
        md_html(f"""<div class="carte-profil"><div style="font-size:38px;">🧑‍🍳</div>
        <h3>Je suis salarié</h3><p style="color:{C_BRUN_CLAIR};font-size:13px;">
        Voir mon planning de la semaine</p></div>""")
    ...
    with c2:
        ...
        Gérer le planning de l'équipe</p></div>""")
    ...
    if st.session_state.get("ecran") == "mdp_responsable":
        ...
        with st.form("form_mdp"):
            ...
            if ok:
                vrai_mdp = obtenir_mot_de_passe_responsable()
                if mdp == vrai_mdp:
